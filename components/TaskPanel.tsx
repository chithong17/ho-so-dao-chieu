...
function TaskForm({ task, answer: a, change, onViewEvidence }: { task: Task; answer: Answer; change: (a: Answer) => void; onViewEvidence?: (id: string) => void }) {
  ...
  const meter = <div className="case-credibility" aria-label={`Độ tin cậy hồ sơ: ${credibility.score}/100, ${credibility.label}`} style={{ border: '1px solid var(--line, rgba(93,66,34,.32))', padding: '10px 12px', margin: '12px 0 18px', borderRadius: '8px' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', fontSize: '12px' }}>
      <span style={{ display: 'inline-flex', alignItems: 'center' }}>
        ĐỘ TIN CẬY HỒ SƠ
        <InfoTooltip text="Đánh giá mức độ chính xác và căn cứ của các lập luận. Sửa nộp lại hoặc xem gợi ý sẽ trừ điểm (-2 điểm/lần)." />
      </span>
  ...
  switch (a.kind) {
    ...
    case 'classify': form = <><div className="classify-info-banner"><span><b>Giải thích mức độ có căn cứ:</b></span><InfoTooltip align="right" text="• Dữ kiện được xác nhận: Sự thật khách quan đã ghi nhận trực tiếp trên thông báo/nhật ký. • Nhận định có cơ sở, cần giới hạn: Có dữ liệu chứng cứ ủng hộ trong phạm vi nhất định, không được suy diễn phóng đại. • Chưa được chứng minh: Giả thuyết, quy chụp cảm tính, chưa có chứng cứ." /></div>{[['s1', 'Demo không còn trong danh sách lúc 19:50.'], ['s2', 'Mã nguồn đã bị xóa.'], ['s3', 'Nam cố ý phá hoại vì bất mãn.'], ['s4', 'Việc rút demo liên quan đến tình trạng chưa kiểm thử đạt.']].map(([id, label], i) => <label className="classify-item" key={id}><span><b>{i + 1}.</b> {label}</span><select aria-label={label} value={a.categories[id] ?? ''} onChange={e => change({ ...a, categories: { ...a.categories, [id]: e.target.value } })}><option value="">Chọn mức độ có căn cứ…</option><option value="fact">Dữ kiện được xác nhận (Đã có chứng cứ khách quan)</option><option value="supported">Nhận định có cơ sở, cần giới hạn (Có chứng cứ, không suy diễn)</option><option value="unproven">Chưa được chứng minh (Suy đoán, thiếu chứng cứ)</option></select></label>)}<EvidencePicker ids={task.evidence} value={a.evidence} onChange={evidence => change({ ...a, evidence })} /></>; break;
    case 'chain': form = <><Sorter label={<>Chuỗi tác động <InfoTooltip text="Chuỗi các mắt xích nhân - quả trực tiếp tạo ra lỗi đăng ký: nguyên nhân này thực sự tác động và sinh ra kết quả tiếp theo, không chỉ xếp theo mốc thời gian." /></>} options={chainOptions} value={a.order} max={4} onChange={order => change({ ...a, order })} /><EvidencePicker ids={task.evidence} value={a.evidence} onChange={evidence => change({ ...a, evidence })} /><Choice label={<>Lỗi và việc rút demo có quan hệ gì? <InfoTooltip text="Phân biệt lỗi kỹ thuật bên trong hệ thống với quyết định tiếp theo của con người." /></>} value={a.relation} options={[{ id: 'same', label: 'Là cùng một sự kiện kỹ thuật.' }, { id: 'decision', label: 'Lỗi chưa khắc phục là căn cứ cho quyết định rút demo tiếp theo.' }]} onChange={relation => change({ ...a, relation })} /></>; break;
    ...
    case 'report': form = <><Sorter label={<>Bố cục giải thích sự cố <InfoTooltip text="Sắp xếp thông tin theo trật tự logic: Nguyên nhân → Hậu quả → Biện pháp → Kiểm thử lại." /></>} options={reportOptions} value={a.order} max={6} onChange={order => change({ ...a, order })} /><Choice label="Vì sao tổ chức lại thông tin?" value={a.reason} options={[{ id: 'structure', label: 'Bố cục và liên kết giữa các phần giúp làm rõ nội dung.' }, { id: 'color', label: 'Hình thức chỉ là chọn màu và phông chữ.' }]} onChange={reason => change({ ...a, reason })} /></>; break;
