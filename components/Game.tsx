...
import {chapters,evidence} from '../game/evidence';
import {counterpoints,initialOptions,tasks} from '../game/tasks';
import {caseCredibility,chapterReadiness,evaluateChallenge} from '../game/engine';
import type {Evidence,Mode} from '../game/types';
...
type EvidenceSource='phone'|'laptop'|'files'|'board';
...
function HomeScreen(){const {state,enter,dispatch,setHome}=useGame();const [assessment,setAssessment]=useState(false),[guide,setGuide]=useState(false),[conclusion,setConclusion]=useState(''),[confidence,setConfidence]=useState(''),[mode,setMode]=useState<Mode>(state.mode),[restart,setRestart]=useState(false);
 const start=(m:Mode,fresh=false)=>{initAudio();setMode(m);enter(m,fresh);setAssessment(true);};
 const hasSave=!!state.initial;
 return <div className="landing"><header className="landing-header"><Brand/><div className="landing-nav"><span className="edition">TRẢI NGHIỆM TRINH THÁM TƯƠNG TÁC</span><button className="text-button" onClick={()=>setGuide(true)}><HelpCircle size={17}/> Cách chơi</button></div></header><main className="landing-main"><section className="hero-copy"><div className="case-label"><span className="status-dot"/> HỒ SƠ ĐANG MỞ <span>№ 001</span></div><h1>Mọi dấu vết<br/>đều có <em>hai mặt.</em></h1><p className="hero-description">Một tin nhắn. Một bản demo bị rút.<br/>Một kết luận quá sớm.</p><p className="hero-detail">Bước vào hồ sơ của nhóm Mạch Nối. Đọc những gì còn sót lại, nối các chứng cứ và tìm sự thật phía sau một đêm triển lãm.</p><div className="hero-actions"><button className="primary hero-cta" onClick={()=>hasSave?setHome(false):start('individual')}><FileSearch size={20}/>{hasSave?'Tiếp tục điều tra':'Mở hồ sơ'}<ArrowRight size={20}/></button><button className="secondary" onClick={()=>start('presenter')}><Users size={18}/> Trình chiếu</button></div>{hasSave&&<button className="text-button new-game" onClick={()=>setRestart(true)}>Bắt đầu lượt mới</button>}<div className="hero-facts"><span><Clock size={15}/> 15–20 phút</span><span><FolderClosed size={15}/> 17 chứng cứ</span><span><ShieldCheck size={15}/> 4 kết thúc</span></div></section><section className="hero-scene" aria-label="Xem trước hồ sơ"><div className="scene-grid"/><div className="case-folder"><div className="folder-tab">HỒ SƠ / HS–01</div><div className="folder-inside"><span>MẠCH NỐI · TRIỂN LÃM SINH VIÊN</span><h2>DEMO<br/>BIẾN MẤT.</h2><div className="folder-rule"/><p>19:50 — Mục trình diễn không còn trong danh sách.</p><div className="classified-stamp">CHƯA KẾT LUẬN</div><div className="barcode"/><small>BẢN LƯU ĐIỀU TRA / 001</small></div></div><div className="hero-message"><div className="mini-window"><span><MessageSquare size={14}/> Mạch Nối Chat</span><small>19:42</small></div><div className="mini-message"><Avatar name="Nam"/><div><strong>Nam <small>Nhóm Mạch Nối</small></strong><p>“Tôi không làm tiếp nữa…”</p><span>Ảnh chụp thiếu phần tiếp theo</span></div></div></div><div className="hero-mail"><span className="mail-icon"><Mail size={22}/></span><div><small>THÔNG BÁO MỚI · 19:50</small><strong>Demo không còn trong danh sách</strong><span>Ban tổ chức triển lãm</span></div><ArrowUpRight size={17}/></div><span className="scene-caption">SỰ THẬT KHÔNG DỪNG Ở ĐIỀU BẠN THẤY.</span></section></main><section className="landing-bottom"><div><span>01</span><p><strong>Đọc giữa những dòng tin.</strong>Hội thoại, email và nhật ký từ cùng một sự cố.</p></div><div><span>02</span><p><strong>Kiểm chứng điều bạn nghĩ.</strong>Tái dựng nguyên nhân, thử những điều kiện khác.</p></div><div><span>03</span><p><strong>Viết lại kết luận.</strong>Chứng cứ quyết định. Bạn có quyền đổi nhận định.</p></div></section><footer className="landing-footer"><span>Nhân vật, dự án và sự kiện đều hư cấu. Tình huống dùng để thực hành phương pháp phân tích.</span><span>PHÉP BIỆN CHỨNG DUY VẬT · 2 / 3 / 6</span></footer>{guide&&<Modal title="Cách mở một hồ sơ" onClose={()=>setGuide(false)}><div className="guide"><p><b>1. Quan sát.</b> Đọc chứng cứ trong các ứng dụng. Ghim tài liệu bạn muốn đối chiếu.</p><p><b>2. Kiểm chứng.</b> Trả lời trong sổ điều tra. Bạn có thể dùng gợi ý, sửa đáp án và đi tiếp dù chưa đúng.</p><p><b>3. Giải mã.</b> Sau mỗi chương, liên hệ lựa chọn với kiến thức và nguồn học thuật.</p><p><b>4. Chốt hồ sơ.</b> Gắn chứng cứ cho kết luận, kiểm tra phương án và xem kết thúc.</p><div className="note">Không có phạt thời gian. Tiến trình lưu riêng trên trình duyệt. Chế độ trình chiếu dùng một máy để cả lớp thảo luận.</div></div></Modal>}{restart&&<Modal title="Bắt đầu lượt mới?" onClose={()=>setRestart(false)}><p>Lượt mới thay tiến trình {state.mode==='presenter'?'trình chiếu':'cá nhân'} trên thiết bị này.</p><div className="button-row"><button className="secondary" onClick={()=>setRestart(false)}>Giữ lượt hiện tại</button><button className="primary" onClick={()=>{setRestart(false);start(state.mode,true);}}>Bắt đầu lượt mới</button></div></Modal>}{assessment&&state.screen==='intro'&&<Modal title="Trước khi mở toàn bộ hồ sơ" onClose={()=>setAssessment(false)}><div className="assessment-grid"><div><EvidenceContent item={evidence[0]} compact/><div className="initial-notice"><Mail size={20}/><div><strong>Thông báo triển lãm · 19:50</strong><p>{evidence[1].summary}</p></div></div></div><div className="assessment-form"><span className="eyebrow">{mode==='presenter'?'CẢ LỚP CÙNG NHẬN ĐỊNH':'NHẬN ĐỊNH BAN ĐẦU'}</span><Choice label="Điều gì có thể đã xảy ra?" options={initialOptions} value={conclusion} onChange={setConclusion}/><Choice label="Bạn tin nhận định này ở mức nào?" options={['Thấp','Vừa','Cao'].map(id=>({id,label:id}))} value={confidence} onChange={setConfidence}/><p className="muted">Không tính điểm. Cuối hồ sơ, bạn sẽ đối chiếu lại nhận định này.</p><button className="primary" disabled={!conclusion||!confidence} onClick={()=>{dispatch({type:'begin',conclusion,confidence});setAssessment(false);}} >Bắt đầu điều tra <ArrowRight size={18}/></button></div></div></Modal>}</div>;
}
function Investigation(){const {state,dispatch,setHome}=useGame();const [filter,setFilter]=useState<'all'|Evidence['app']>('all'),[query,setQuery]=useState(''),[view,setView]=useState<'evidence'|'timeline'|'board'>('evidence'),[mobileTab,setMobileTab]=useState('read');
 ...
 const handleInteract = (objId: string) => {
   ...
   if(!source)return;
   const first=sourceRecords(source)[0];
   if(first)dispatch({type:'evidence',id:first.id});
 ...
 const sourceLibrary=(source:EvidenceSource,title:string,description:string)=><aside className="evidence-library" style={{flex:1,borderRight:'none',maxWidth:'330px'}}>
   <div className="library-head"><strong>{title}</strong><span>{sourceRecords(source).length}<small> / {Object.values(evidenceSources).filter(v=>v===source).length}</small></span></div>
      ...
      <>
        <div className="phone-search-row" style={{display:'flex', alignItems:'center', padding:'6px'}}>
          <label className="search-box" style={{flex:1, display:'flex', alignItems:'center', border:'1px solid rgba(85,255,85,0.4)', borderRadius:'8px', padding:'3px 6px', margin:0}}>
           ...
           <span className="count" style={{fontSize:'8px', color:'rgba(85,255,85,0.7)', marginLeft:'8px', whiteSpace:'nowrap'}}>{sourceRecords('phone').length} / 4</span>
        </div>
...
export default function Game(){ useEffect(() => { const handleGlobalClick = (e: any) => { const target = e.target; const sfxTarget = target.closest("[data-sfx]"); if (sfxTarget) { ensureBgmPlaying();
          playSfx(sfxTarget.getAttribute("data-sfx")); } else if (target.closest("button") || target.closest("a")) { ensureBgmPlaying();
          playSfx("click"); } }; window.addEventListener("click", handleGlobalClick, true); return () => window.removeEventListener("click", handleGlobalClick, true); }, []); return <GameProvider><ChapterEvents/><Shell/></GameProvider>; }
