...
async function fillTask(page: Page, id: string, a: Answer) {
    ...
    switch (a.kind) {
        ...
        case 'chain': for (const v of a.order) await panel.getByRole('button', { name: chainOptions.find(o => o.id === v)!.label, exact: true }).click(); await radio('Lỗi chưa khắc phục là căn cứ cho quyết định rút demo tiếp theo.'); break;
    ...
    if (id === 'T11') { await panel.getByRole('button', { name: 'Mô phỏng phương án' }).click(); await expect(panel.getByText(/demo giới hạn qua test/)).toBeVisible(); }
