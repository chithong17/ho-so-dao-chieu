...
const unaccepted=()=>reduceGame(createState('individual','standard'),{type:'begin',conclusion:'unknown',confidence:'Vừa'});
const started=()=>{const state=unaccepted();return reduceGame(state,{type:'acceptMission',id:state.activeTask});};
describe('Điểm thi đấu',()=>{
 it('không tính điểm chứng cứ trước khi nhận nhiệm vụ',()=>{const state=unaccepted(),action={type:'evidence',id:'E01'} as const;expect(scoreAction(state,action,applyCompetitionAction(state,action,'x'),'a')).toEqual([]);});
